#-------------------------------------------------------------------------------
# Name:        nec_sp350_CallControl
# Purpose:     This is the nec_sp350-specific iUI_CallControl interface class,
#              derived from the core iUI_CallControl class.
#
...
class Nec_sp350_CallControl(uicc.UI_CallControl):

    def PlaceCall(self, operation_type, contact):
        status = ccc.STATUS_SUCCESS
        methodname = 'PlaceCall'

        super(Nec_sp350_CallControl, self).log_method_called(self.phone_name, methodname)

        # Validate operation type for function and for this softphone
        if not isvalid_func_op(ccc.OT_PLACE_CALL, operation_type):
    ...
    def AnswerCall(self, operation_type):
        ...
        # Validate operation type for function and for this softphone
        if not isvalid_func_op(ccc.OT_ANSWER_CALL, operation_type):
            status = ccc.STATUS_INVALID_OPERATION_TYPE
        else:
            if operation_type in [ccc.OT_TOAST, ccc.OT_PICKUP]:
                status = ccc.STATUS_FEATURE_NOT_SUPPORTED
    ...
    def DeclineCall(self, operation_type):
        ...
        # Validate operation type for function and for this softphone
        if not isvalid_func_op(ccc.OT_DECLINE_CALL, operation_type):
            status = ccc.STATUS_INVALID_OPERATION_TYPE
        else:
            if operation_type in [ccc.OT_TOAST, ccc.OT_MAIN_WND]:
                status = ccc.STATUS_FEATURE_NOT_SUPPORTED
    ...
    def EndCall(self, operation_type):
        ...
        # Validate operation type for function and for this softphone
        if not isvalid_func_op(ccc.OT_END_CALL, operation_type):
            status = ccc.STATUS_INVALID_OPERATION_TYPE
        else:
    ...
    def MuteHoldTransferCall(self, operation_type):
        ...
        # Validate operation type for function and for this softphone
        if not isvalid_func_op(ccc.OT_MUTE_HOLD_TRANSFER_CALL, operation_type):
            status = ccc.STATUS_INVALID_OPERATION_TYPE
        else:
            if operation_type in [ccc.OT_MUTE,
                                ccc.OT_UNMUTE,
                                ccc.OT_TRANSFER_AUDIO_HS_TO_PC,
    ...
    def TwoCalls(self, operation_type, contact):
        ...
        # Validate operation type for function and for this softphone
        if not isvalid_func_op(ccc.OT_TWO_CALLS, operation_type):
            status = ccc.STATUS_INVALID_OPERATION_TYPE
        else:
            if operation_type in [ccc.OT_TWO_CALLS_ACTIVEMUTED_DIALING,
                                ccc.OT_TWO_CALLS_ONHOLDMUTED_DIALING]:
                status = ccc.STATUS_FEATURE_NOT_SUPPORTED
    ...
    def ConferenceCall(self, operation_type):
        ...
        # Validate operation type for function and for this softphone
        if not isvalid_func_op(ccc.OT_CONFERENCE_CALL, operation_type):
            status = ccc.STATUS_INVALID_OPERATION_TYPE
        else:
