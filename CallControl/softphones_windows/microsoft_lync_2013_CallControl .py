...
class Microsoft_lync_2013_CallControl(uicc.UI_CallControl):
    ...
    def PlaceCall(self, operation_type, contact_id):
        ...
        if status == ccc.STATUS_SUCCESS:
            ...
            if operation_type == ccc.OT_CONTACT_NAME:
                ...
                try:
                    ...
                    squish.clickButton(squish.waitForObject("{container=self.lync_window text='Contacts' role='PushButton' type='Button'}"))
        ...
        if status == ccc.STATUS_SUCCESS:
            ...
            try:
                contact_phonified = self.phonify(contact_id)
                call_status_label = squish.waitForObject("{text='Calling %s...' type='Label'}" % contact_phonified)
        ...
        if status == ccc.STATUS_SUCCESS:
            if operation_type == ccc.OT_TRANSFER_CALL:
                ...
                if status == ccc.STATUS_SUCCESS:
                    ...
                    try:
                        squish.mouseClick("{name='Transfer Call' role='PageTab'}")
                        test.log('Clicked on \'Transfer Call\' tab')
                    except LookupError:
