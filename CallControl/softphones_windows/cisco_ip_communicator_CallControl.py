#-------------------------------------------------------------------------------
# Name:        cisco_ip_communicator_CallControl
# Purpose:     This is the cisco_ip_communicator-specific iUI_CallControl
...
class Cisco_ip_communicator_CallControl(uicc.UI_CallControl):

    def PlaceCall(self, operation_type, contact):
        status = ccc.STATUS_SUCCESS
        methodname = 'PlaceCall'

        super(Cisco_ip_communicator_CallControl, self).log_method_called(self.phone_name, methodname)
        ...
        if not isvalid_func_op(ccc.PLACE_CALL, operation_type):
            ...
            if operation_type in [ccc.OT_CONTACT_NAME,
    ...
    def AnswerCall(self, operation_type):
        ...
        if status == ccc.STATUS_SUCCESS:
           if operation_type == ccc.OT_TOAST:
                ...
                pass
           elif operation_type == ccc.OT_PICKUP:
                pass
           else:
                # Should never get here, given the above checks... being safe
                status = ccc.STATUS_INVALID_OPERATION_TYPE
    ...
    def TwoCalls(self, operation_type, contact):
        ...
        if not isvalid_func_op(ccc.TWO_CALLS, operation_type):
            ...
            if operation_type in [ccc.OT_TWO_CALLS_ACTIVE_DIALING,
                                ccc.OT_TWO_CALLS_ACTIVEMUTED_DIALING,
                                ccc.OT_TWO_CALLS_INCOMING_DIALING,
