#-------------------------------------------------------------------------------
# Name:        ibm_sametime_CallControl
# Purpose:     This is the ibm_sametime-specific iUI_CallControl interface
...
class Ibm_sametime_CallControl(uicc.UI_CallControl):

    def PlaceCall(self, operation_type, contact):
        status = ccc.STATUS_SUCCESS
        methodname = 'PlaceCall'

        super(Ibm_sametime_CallControl, self).log_method_called(self.phone_name, methodname)
    ...
    def AnswerCall(self, operation_type):
        ...
        if not isvalid_func_op(ccc.ANSWER_CALL, operation_type):
            ...
            if operation_type in[ccc.OT_MAIN_WND, ccc.OT_PICKUP]:
                status = ccc.STATUS_FEATURE_NOT_SUPPORTED
    ...
    def ConferenceCall(self, operation_type):
        ...
        if not isvalid_func_op(ccc.CONFERENCE_CALL, operation_type):
            ...
            if operation_type in[ccc.OT_CONFERENCE_TWO_CALLS,
